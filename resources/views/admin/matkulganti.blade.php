<x-layout>
    <h2 class="mt-5 text-3xl font-semibold pt-8 lg:pt-0 text-gray-900 dark:text-white">Data Matkul Ganti</h2>
...
<div id="confirmationModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <p>Apakah Anda yakin ingin menghapus matkul ganti ini?</p>
...
<div id="createGedung" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-20 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <!-- Modal body -->
            <form id="createGedungForm" method="POST" action="{{ route('admin.matkulganti.store') }}" enctype="multipart/form-data">
                @csrf
                <div class ="grid gap-4 mb-4   grid-cols-2">
                    <div>
                        <label for="matkul" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">matakuliah</label>
                        <select name="matakuliah" id="matakuliah" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                            @foreach($matkul as $item) <!-- Ganti $item menjadi $gedungs -->
                    ...
                    <div class="relative max-w-sm">
                        ...
                        <input id="datepicker-ganti-@foreach ($kelas as $klas ){{ $klas->nomorkelas }}@endforeach" name="tanggalganti" datepicker datepicker-buttons datepicker-autoselect-today type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date"> 
                    </div>
                    <div class="relative max-w-sm">
    ...
    <div class="overflow-hidden overflow-x-auto rounded-lg border border-gray-200 shadow-md m-5">
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
          <thead class="bg-gray-50 text-center">
            <tr>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Matkul</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Nomor Kelas</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">tanggalganti</th>
          ...
          <tbody class="divide-y divide-gray-100 border-t border-gray-100">
            @foreach ($matkulganti as $g )
                
           
            <tr class="hover:bg-gray-50">
              <th class="flex gap-3 px-6 py-4 font-normal text-gray-900 text-center" >
                <div class="text-sm">
                  <div class="font-medium text-gray-700">{{ $g->matakuliah }}</div>
                </div>
              </th>
              <td class="px-6 py-4 text-center">{{ $g->nomorkelas }}</td>
              <td class="px-4 py-3 font-semibold">{{ \Carbon\Carbon::parse($g->tanggalganti)->translatedFormat('d F') }}</td>
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex justify-end gap-4">
                    ...
                    <form id="delete-form-{{ $g->id }}" action="{{ route('admin.matkulganti.destroy', $g->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
            ...
            <td>
                 ...
                 data-id="{{ $g->id }}"
                 data-matakuliah="{{ $g->matakuliah }}"
                 data-nomorkelas="{{ $g->nomorkelas }}"
                 data-nomorkelas="{{ $g->tanggalganti }}"
                 data-hari="{{ $g->hari }}"
...
<div id="editGedung" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-20 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form id="editGedungForm" method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 mb-4   grid-cols-2">
                     <!-- Input kelas -->
     <div>
        <label for="matakuliah" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Matkul</label>
        <select name="matakuliah" id="Matakuliah" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
            @foreach($matkul as $item) <!-- Ganti $item menjadi $gedungs -->
            <option value="{{ $item->matakuliah }}">{{ $item->matakuliah }}</option>
        @endforeach
    ...
    <div class="relative max-w-sm">
        ...
        <input id="datepicker-ganti-@foreach ($kelas as $noklas){{ $noklas->nomorkelas }}@endforeach" name="tanggalganti" datepicker datepicker-buttons datepicker-autoselect-today type="date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date"> 
    </div>
                    <div class="relative max-w-sm">
                        <label for="Hari" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pilih Hari</label>
                        <div class="absolute start-0 flex items-center ps-3 py-3 pointer-events-none">
...
<div class="flex justify-center mt-5 mb-5"> 
    @if ($matkulganti->hasPages())
<nav aria-label="Page navigation example">
<ul class="inline-flex -space-x-px text-base h-10">
    {{-- Previous Page Link --}}
    @if ($matkulganti->onFirstPage())
    ...
    @else
        <li>
            <a href="{{ $matkulganti->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
        </li>
    @endif

    {{-- Pagination Elements --}}
    @foreach ($matkulganti->links()->elements as $element)  {{-- Perubahan di sini --}}
        ...
        @if (is_array($element))
            @foreach ($element as $page => $url)
                @if ($page == $matkulganti->currentPage())
    ...
    {{-- Next Page Link --}}
    @if ($matkulganti->hasMorePages())
        <li>
            <a href="{{ $matkulganti->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
        </li>
    @else
        <li aria-disabled="true">
...
<script>
   document.querySelectorAll('[data-modal-toggle="editGedung"]').forEach(button => {
    button.addEventListener('click', () => {
        ...
        const matakuliah = button.getAttribute('data-matakuliah');
        ...
        editForm.action = `/admin/matkulganti/${id}`; // Adjust the route as necessary
