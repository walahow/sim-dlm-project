<x-layout>
    <h2 class="mt-5 text-3xl font-semibold pt-8 lg:pt-0 text-gray-900 dark:text-white">Data Matkul</h2>
...
<div id="createGedung" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-20 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <!-- Modal body -->
            <form id="createGedungForm" method="POST" action="{{ route('admin.matkul.store') }}" enctype="multipart/form-data">
                @csrf
        ...
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
          ...
          <tbody class="divide-y divide-gray-100 border-t border-gray-100">
            ...
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 text-center">{{ $g->matakuliah }}</td>
              ...
              <td class="px-6 py-4 text-center">{{ $g->dosen}}</td>
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex justify-end gap-4">
                    ...
                    <form id="delete-form-{{ $g->id }}" action="{{ route('admin.matkul.destroy', $g->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
...
<ul class="inline-flex -space-x-px text-base h-10">
    ...
    @else
        <li>
            <a href="{{ $matkul->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
        </li>
    @endif

    {{-- Pagination Elements --}}
    @foreach ($matkul->links()->elements as $element)  {{-- Perubahan di sini --}}
    ...
    @if ($matkul->hasMorePages())
        <li>
            <a href="{{ $matkul->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
        </li>
    @else
        <li aria-disabled="true">
...
<script>
   document.querySelectorAll('[data-modal-toggle="editGedung"]').forEach(button => {
    button.addEventListener('click', () => {
        ...
        const tanggalpesan = button.getAttribute('data-tanggalpesan');
        ...
        editForm.action = `/admin/matkul/${id}`; // Adjust the route as necessary

        // Populate the input fields in the edit modal
        document.getElementById('Nomorkelas').value = nomorkelas;
        document.getElementById('Username').value = username;
        document.getElementById('Tanggalpesan').value = tanggalpesan;
