...
<div id="createGedung" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-20 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <!-- Modal body -->
            <form id="createGedungForm" method="POST" action="{{ route('admin.gedung.store') }}"enctype="multipart/form-data">
                @csrf
                <div class ="grid gap-4 mb-4   grid-cols-2">
                    <div>
                        <label for="Nomorgedung" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomor Gedung</label>
                        <input type="text" name="nomorgedung" id="Nomorgedung" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                    </div>
                    <div>
                        <label for="Jumlahkelas" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Kelas</label>
                        <input type="number" name="jumlahkelas" id="Jumlahkelas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="PengurusGedung" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pengurus Gedung</label>
                        <input type="text" name="Pengurusgedung" id="PengurusGedung" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                    </div>
                    <div>
                        <label for="Lantai" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Lantai</label>
                        <input type="number" name="lantai" id="Lantai" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                    </div>
                    <div>
    ...
    <div class="overflow-hidden overflow-x-auto rounded-lg border border-gray-200 shadow-md m-5">
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
          <thead class="bg-gray-50 text-center">
            <tr>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Gedung</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Jumlah Kelas</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Lokasi</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">pengurus gedung</th>
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">Lantai</th>
          ...
          <tbody class="divide-y divide-gray-100 border-t border-gray-100">
            ...
            <tr class="hover:bg-gray-50">
              <th class="flex gap-3 px-6 py-4 font-normal text-gray-900 text-center" >
                ...
                <div class="text-sm">
                  <div class="font-medium text-gray-700">{{ $g->nomorgedung }}</div>
                </div>
              </th>
              <td class="px-6 py-4 text-center">{{ $g->jumlahkelas }}</td>
              <td class="px-6 py-4 text-center">{{ $g->lokasi }}</td>
              <td class="px-6 py-4 text-center">
                {{ $g->Pengurusgedung }}
              </td>
              <td class="px-6 py-4 text-center">{{ $g->lantai }}</td>

              <td class="px-6 py-4 text-center">
                <div class="flex justify-end gap-4">
                    <!-- Form untuk menghapus gedung -->
                    <form id="delete-form-{{ $g->id }}" action="{{ route('admin.gedung.destroy', $g->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
            ...
            <td>
                 ...
                 data-jumlahkelas="{{ $g->jumlahkelas }}"
                 data-lokasi="{{ $g->lokasi }}"
                 data-Pengurusgedung="{{ $g->Pengurusgedung }}"
                 data-lantai="{{ $g->lantai }}"
...
<ul class="inline-flex -space-x-px text-base h-10">
    ...
    @else
        <li>
            <a href="{{ $gedung->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
        </li>
    @endif

    {{-- Pagination Elements --}}
    @foreach ($gedung->links()->elements as $element)  {{-- Perubahan di sini --}}
    ...
    @if ($gedung->hasMorePages())
        <li>
            <a href="{{ $gedung->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
        </li>
    @else
        <li aria-disabled="true">
...
<script>
   document.querySelectorAll('[data-modal-toggle="editGedung"]').forEach(button => {
    button.addEventListener('click', () => {
        ...
        const jumlahkelas = button.getAttribute('data-jumlahkelas');
        const lokasi = button.getAttribute('data-lokasi');
        const pengurusgedung = button.getAttribute('data-Pengurusgedung');
        ...
        editForm.action = `/admin/gedung/${id}`; // Adjust the route as necessary

        // Populate the input fields in the edit modal
        document.getElementById('nomorgedung').value = nomorgedung;
        document.getElementById('jumlahkelas').value = jumlahkelas;
        document.getElementById('lokasi').value = lokasi;
        document.getElementById('Pengurusgedung').value = pengurusgedung;
